...
public abstract class Literal extends Expression implements LeafExpression {
    ...
    protected Expression throwOrNull(boolean strict, DataType targetType) {
        if (strict) {
            throw new AnalysisException(String.format("%s can't cast to %s in strict mode.", getValue(), targetType));
        } else {
            return new NullLiteral(targetType);
        }
    }

    protected Expression throwOrNull(boolean strict, DataType targetType, AnalysisException e) {
        if (strict) {
            throw e;
        } else {
            return new NullLiteral(targetType);
    ...
    protected Expression uncheckedCastTo(DataType targetType) throws AnalysisException {
        ...
        //                 desc.substring(0, pointZeroIndex), ((VarcharType) targetType).getLen());
        //         }
        //     }
        ...
        throw new RuntimeException(String.format("FE doesn't support cast from %s to %s", this, targetType));
    }
