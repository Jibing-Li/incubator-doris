...
public abstract class StringLikeLiteral extends Literal implements ComparableLiteral {
    ...
    public final String toDecimalRegex = "^\\s*[+-]?((\\d+\\.\\d+)|(\\d+)|(\\d+\\.)|(\\.\\d+))(?:(e|E)[+-]?\\d+)?\\s*$";
    ...
    protected Expression uncheckedCastTo(DataType targetType) throws AnalysisException {
        ...
        if (targetType instanceof DateType || targetType instanceof DateV2Type) {
            ...
            return castToDecimal(targetType, strictCast);
        } else if (targetType.isDecimalV3Type()) {
            return castToDecimal(targetType, strictCast);
        } else if (targetType.isBooleanType()) {
            return castToBool(targetType, strictCast);
        }
    ...
    protected Expression castToFloat(boolean strictCast) {
        try {
            ...
            return throwOrNull(strictCast, FloatType.INSTANCE, e);
        }
    }
    ...
    protected Expression castToDouble(boolean strictCast) {
        try {
            ...
            return throwOrNull(strictCast, DoubleType.INSTANCE, e);
        }
    }

    protected Expression castToBool(DataType targetType, boolean strictCast) {
        try {
            String trimmedValue = value.trim();
            ...
            throw new AnalysisException(String.format("%s can't cast to bool in strict mode.", value));
        } catch (AnalysisException e) {
            return throwOrNull(strictCast, targetType, e);
        }
    }

    protected Expression castToDecimal(DataType targetType, boolean strictCast) {
        try {
            Pattern pattern = Pattern.compile(toDecimalRegex);
            String trimmedValue = value.trim();
            if (pattern.matcher(trimmedValue).matches()) {
                BigDecimal bigDecimal = new BigDecimal(trimmedValue);
                return getDecimalLiteral(bigDecimal, targetType);
            }
            throw new AnalysisException(String.format("%s can't cast to decimal in strict mode.", value));
        } catch (AnalysisException e) {
            return throwOrNull(strictCast, targetType, e);
        }
    }
